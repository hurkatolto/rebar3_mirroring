...
%% @doc
%%
%% Mirroring module for hg resources.
%%
%% @end

-module(rebar3_hg_mirroring).
...
lock(Dir, Source) ->
    rebar_hg_resource:lock(Dir, Source).
...
download(Dir, Source, State) ->
    rebar_hg_resource:download(Dir, Source, State).
...
needs_update(Dir, Source) ->
    rebar_hg_resource:make_vsn(Dir, Source).

-spec make_vsn(Dir) -> Res when
    Dir :: file:filename_all(),
    Res :: {plain, string()} | {error, string()}.
make_vsn(Dir) ->
    rebar_hg_resource:make_vsn(Dir).
